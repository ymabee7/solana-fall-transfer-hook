...
pub fn handler<'info>(
    ...
    // 1. a plain transfer_checked, no hook accounts yet
    ...
    // 2. the infos for those accounts, in the same order
    let mut infos = vec![
        source.clone(),
        mint.clone(),
        destination.clone(),
    ...
    // 3. let the helper read the meta list and append what the hook asked for
